...
class PositionBehavior(ABC):
    """Abstract base class for calculating positions."""

    @abstractmethod
    def calculate_pnl(self, position, close_price, amount):
        """Abstract method for calculating profit and loss."""

    @abstractmethod
    def calculate_liquidation_price(self, position, price):
        """Abstract method for calculating liquidation price."""

    @abstractmethod
    def check_for_sl(self, position, row):
        """Abstract method for checking if stop-loss condition was hit."""

    @abstractmethod
    def check_for_tp(self, position, row):
        """Abstract method for checking if profit should be taken."""

    @abstractmethod
    def check_for_liquidation(self, position, row):
        """Abstract method for checking if position has to be liquidated."""


class LongPositionBehavior(PositionBehavior):
    """Class used for calculating long positions."""
...
class ShortPositionBehavior(PositionBehavior):
    """Class used for calculating short positions."""
...
class Position:
    """Represents a leveraged trading position with PnL, SL, TP, and liquidation tracking."""
    ...
    def __init__(
        ...
        self.close_fee_rate = fee_rate if open_fee_rate is None else close_fee_rate
        self.side = None
        self.open_time = None
